...
const sectionWorldNews = (update)=>{
  ...
  $.get("/api/news/", (response, status)=>{
    ...
    let item1 = content(worldNews[9].title, worldNews[9].brief, 'assets/img/' + worldNews[9].url, 'second-news__contain-img col-xs-12 p-0', 'col-xs-12 in-flex-sm');
    item1.addClass('col-xs-12 second-news world-news--content-sm p-0');
    colLeft.append(item1);
      
    let item2 = content(worldNews[1].title, worldNews[1].brief, 'assets/img/' + worldNews[1].url, 'second-news__contain-img col-xs-6 col-sm-12 p-0 in-flex-sm in-flex-sm-col', 'col-xs-6 col-sm-12 in-flex-xs in-flex-sm');
    item2.addClass(' col-xs-12 second-news col-sm-6 world-news--cont-middle-up p-t-15');
      
    let item3 = content(worldNews[2].title, worldNews[2].brief, 'assets/img/' + worldNews[2].url, 'second-news__contain-img col-xs-6 col-sm-12 p-0 in-flex-sm in-flex-sm-col', 'col-xs-6 col-sm-12 in-flex-xs in-flex-sm');
    item3.addClass('col-xs-12 second-news col-sm-6 world-news--cont-middle-up p-t-15 p-l-0');
      
    let item4 = content(worldNews[3].title, worldNews[3].brief, 'assets/img/' + worldNews[3].url, 'second-news__contain-img col-xs-6 col-sm-5 p-0 in-flex-sm in-flex-xs', 'col-xs-6 col-sm-7 in-flex-xs');
    item4.addClass('col-xs-12 second-news world-news--cont-middle-down p-t-15');
      
    let item5 = content(worldNews[4].title, worldNews[4].brief, 'assets/img/' + worldNews[4].url, 'second-news__contain-img col-xs-6 col-sm-5 p-0 in-flex-sm in-flex-xs', 'col-xs-6 col-sm-7 in-flex-xs');
    item5.addClass('col-xs-12 second-news hidden-xs world-news--cont-middle-down p-t-15');
      
    colMidle.append(item2, item3, item4, item5);
      
    let item6 = content(worldNews[5].title, worldNews[5].brief, 'assets/img/' + worldNews[5].url, 'second-news__contain-img col-xs-12 p-0', 'col-xs-12 in-flex-sm');
